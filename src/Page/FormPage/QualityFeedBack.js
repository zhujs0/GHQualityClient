...
import {
    ...
    Modal, Layout, Breadcrumb, Affix,Tabs,Divider
} from 'antd';
import 'antd/dist/antd.min.css';
import 'whatwg-fetch';
import { QualityHost } from '../config';
import $ from 'jquery';
import QualityFeedBackDetails from './QualityFeedBackDetails'
...
class QualityFeedBack extends Component {
    constructor() {
        ...
        this.state = {
            loading: true,
            panes:[{ title: '质量反馈单', 
            content: <Main ParentFun={(OrderNo)=>{this.ShowDetails(OrderNo)}} NewControl={()=>{this.NewControl()}} />,
             key: "FeedList" }],
            activeKey: "FeedList"
        }
    }
    ...
    ShowDetails(OrderNo) {
        ...
        panes.push(
            { title: OrderNo + '反馈单详情', content: <QualityFeedBackDetails  ControlNo={OrderNo} />, key: OrderNo.trim() }
        );
    ...
    NewControl()
    {
        var panes = this.state.panes;
        panes.push(
            { title: '新建', content: <QualityFeedBackDetails NewControl="True" />, key: 'NewTabs' }
        );
...
class Main extends Component {
                ...
                switch (value) {
                    case "0": case "1":
                        if (sessionStorage.getItem("ConfirmStepID") === record.stepID) {
                            if (record.isControl === '1') {
                                return (<spec style={{ color: "blue" }}>已生成控制表，请确认<Icon type="star-o" /></spec>);
                            }
                    ...
                    case "2":
                        if (record.isControl === '1') {
                            return (<spec style={{ color: "#00ba00" }}>已生成质量控制表<Icon type="smile-o" /></spec>)
                        }
    ...
    GetSelect(OrderNo, BatchNo, WorkProcedure, StartTime, EndTime, EquipmentNo, Model, FeedbackMan, Status, PageIndex, intPageSize, pagination) {
        fetch(QualityHost + "/api/Order?OrderNo=" + OrderNo + "&BatchNo=" + BatchNo + "&WorkProcedure=" + WorkProcedure + "&StartTime=" + StartTime + "&EndTime=" + EndTime
            ...
            + Status + "&PageIndex=" + PageIndex + "&PageSize=" + intPageSize + "&ActionType=GetOrderInfo&OrderType=0"
        ).then(res => {
            if (res.status === 200) {
                return res.json();
            }
    ...
    render() {
        ...
        return (
            ...
            <div style={{fontSize:"9pt" }}>
                {/* 查询 */}
                <div style={{}}>
                    <table style={{ lineHeight: "38px", fontWeight: "600" }}>
                        <tbody>
                            <tr>
                                ...
                                <td style={{ width: "60px", textAlign: "right" }}>工序：</td>
                                <td>
                                ...
                                <td style={{ textAlign: "right", width: "90px" }}>型号规格：</td>
                                <td><Input id="Model" style={{ width: "150px", marginLeft: "5px" }}></Input></td>
                                <td style={{ width: "90px", textAlign: "right", display: "" }}>反馈时间：</td>
...
//const NewFeedBackCom = connect(mapStateToProps)(NewFeedBack);
